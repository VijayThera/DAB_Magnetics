...
class DABStackedTransformerOptimization:
    ...
    def objective(trial, config: DABStoSingleInputConfig,
        ...
        window_w = trial.suggest_float("window_w",
                                       config.window_w_min_max_list[0],
                                       config.window_w_min_max_list[1])

        air_gap_transformer = trial.suggest_float("air_gap_transformer", 0.1e-3, 1e-3)
        ...
        secondary_litz_wire = trial.suggest_categorical("primary_litz_wire", config.secondary_litz_wire_list)
        secondary_litz_parameters = ff.litz_database()[secondary_litz_wire]
        secondary_litz_diameter = 2 * secondary_litz_parameters["conductor_radii"]
        ...
        #primary_coil_turns = trial.suggest_int("primary_coil_turns",
        #                                       config.primary_coil_turns_min_max_list[0],
        ...
        try:
            ...
            insulation, coil_window, transformer_window = femmt.functions_topologies.set_DAB_stacked_transformer(
                core=core,
                ...
                # primary litz
                primary_conductor_radius=primary_litz_parameters["conductor_radii"],
                primary_number_strands=primary_litz_parameters["strands_numbers"],
                primary_strand_radius=primary_litz_parameters["strand_radii"],
                primary_fill_factor=config.primary_fill_factor,
                N_p_top=N_p_top,
                N_p_bot=N_p_bot,

                # secondary litz
                secondary_conductor_radius=secondary_litz_parameters["conductor_radii"],
                secondary_number_strands=secondary_litz_parameters["strands_numbers"],
                secondary_strand_radius=secondary_litz_parameters["strand_radii"],
                secondary_fill_factor=config.secondary_fill_factor,
                N_s_top=N_s_top,
                N_s_bot=N_s_bot,
            ...
            # read result-log
            with open(source_json_file, "r") as fd: loaded_data_dict = json.loads(fd.read())
            ...
            if number_objectives == 3:
                return (total_volume, total_loss,
                        100 * (abs(difference_l_h / config.l_h_target) + abs(difference_l_s12 / config.l_s12_target)))
            elif number_objectives == 4:
                return total_volume, total_loss, 100 * abs(difference_l_h), 100 * abs(difference_l_s12)
    ...
    @staticmethod
    def start_proceed_study(study_name: str, config: DABStoSingleInputConfig, number_trials: int,
        ...
        target_and_fixed_parameters = femmt.optimization.DABStackedTransformerOptimization.calculate_fix_parameters(config)
        ...
        func = lambda trial: femmt.optimization.DABStackedTransformerOptimization.objective(trial,
                                                                                            config,
        ...
        study_in_memory.optimize(func, n_trials=number_trials, show_progress_bar=True,
                                 callbacks=[femmt.DABStackedTransformerOptimization.run_garbage_collector])
    ...
    @staticmethod
    def proceed_multi_core_study(study_name: str, config: DABStoSingleInputConfig, number_trials: int,
                                 number_objectives: int = None,
        ...
        func = lambda trial: femmt.optimization.DABStackedTransformerOptimization.objective(trial, config, target_and_fixed_parameters,
    ...
    @staticmethod
    def show_study_results(study_name: str, config: DABStoSingleInputConfig,
    ...
    @staticmethod
    def show_study_results3(study_name: str, config: DABStoSingleInputConfig,
    ...
    @staticmethod
    def re_simulate_single_result(study_name: str, config: DABStoSingleInputConfig, number_trial: int,
    ...
    @staticmethod
    def re_simulate_from_df(df: pd.DataFrame, config: DABStoSingleInputConfig, number_trial: int,
                            fft_filter_value_factor: float = 0.01, mesh_accuracy: float = 0.5,
    ...
    @staticmethod
    def save_png_from_df(df: pd.DataFrame, config: DABStoSingleInputConfig, number_trial: int):
        """
    ...
    @staticmethod
    def create_full_report(df: pd.DataFrame, trials_numbers: list[int], config: DABStoSingleInputConfig,
                           thermal_config: ThermalConfig,
                           current_waveforms_operating_points: List[DABCurrentWorkingPoint],
                           fft_filter_value_factor: float = 0.01, mesh_accuracy: float = 0.5):
        ...
        for trial_number in trials_numbers:
            ...
            for count_current_waveform, current_waveform in enumerate(current_waveforms_operating_points):
                ...
                geo_sim = femmt.DABStackedTransformerOptimization.re_simulate_from_df(df, config,
                                                                                   number_trial=trial_number,
                                                                                   show_simulation_results=False,
                                                                                   fft_filter_value_factor=fft_filter_value_factor,
                                                                                   mesh_accuracy=mesh_accuracy)
                # perform the thermal simulation
                femmt.DABStackedTransformerOptimization.thermal_simulation_from_geo(geo_sim, thermal_config,
    ...
    @staticmethod
    def create_pngs(df: pd.DataFrame, trials_numbers: list[int], config: DABStoSingleInputConfig):
        """
        ...
        for trial_number in trials_numbers:
            femmt.DABStackedTransformerOptimization.save_png_from_df(df, config, number_trial=trial_number, show_simulation_results=False)
